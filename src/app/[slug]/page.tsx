...
function getClientSectionCopy(tradeName: string, businessName: string) {
  ...
  if (culinary) {
    return {
      trustTitle: 'Una experiencia cuidada desde el primer contacto.',
      trustSub: 'Atención clara, propuesta personalizada y una presentación pensada para que cada invitado disfrute sin improvisaciones.',
      serviceTitle: 'Servicios gastronómicos para cada ocasión.',
      serviceSub: `Elige el tipo de experiencia que necesitas y habla directamente con ${businessName} para confirmar disponibilidad.`,
      processTitle: 'De la idea al menú, todo claro desde el inicio.',
      processSub: 'Primero entendemos el evento, después proponemos una experiencia gastronómica adaptada a tus invitados.',
      galleryTitle: 'Platos, espacios y detalles que hablan por sí solos.',
      gallerySub: 'Imágenes seleccionadas para transmitir el tipo de experiencia que recibirá cada cliente.',
      faqTitle: 'Preguntas frecuentes antes de reservar.',
      aboutKicker: 'Quiénes somos',
      experienceTitle: 'Menús, eventos y atención a medida.',
      experienceSub: 'Una propuesta gastronómica no empieza en la cocina: empieza entendiendo el momento, los invitados y el nivel de detalle que quieres transmitir.',
    }
  }
  return {
    trustTitle: 'Confianza, claridad y un resultado bien cuidado.',
    trustSub: 'Una forma de trabajar seria, cercana y pensada para que el cliente sepa desde el principio qué va a recibir.',
    serviceTitle: 'Servicios profesionales adaptados a cada necesidad.',
    serviceSub: `Elige el servicio que necesitas y habla directamente con ${businessName} para recibir orientación sin compromiso.`,
    processTitle: 'Así trabajamos de principio a fin.',
    processSub: 'Un proceso claro para que cada cliente entienda tiempos, pasos y resultado antes de empezar.',
    galleryTitle: 'Trabajos, detalles y resultados reales.',
    gallerySub: 'Una galería cuidada para que puedas ver el estilo, la calidad y la forma de trabajar.',
    faqTitle: 'Preguntas frecuentes antes de contactar.',
    aboutKicker: 'Quiénes somos',
    experienceTitle: 'Atención profesional con trato cercano.',
    experienceSub: 'Cada servicio se prepara con comunicación clara, cuidado en los detalles y una respuesta pensada para resolver lo que necesitas.',
  }
}
    ...
    <>
      <style>{`
        ...
        *{box-sizing:border-box}html{scroll-behavior:smooth}body{margin:0;overflow-x:hidden;background:var(--cream);font-family:Inter,system-ui,sans-serif;color:var(--ink)}a{text-decoration:none;color:inherit}img{max-width:100%;display:block}button{font:inherit}
        ...
        .faq{max-width:960px;margin:auto;display:grid;gap:16px}.faq-item{background:white;border-radius:24px;border:1px solid rgba(0,0,0,.08);box-shadow:0 16px 50px rgba(0,0,0,.06);overflow:hidden}.faq-q{width:100%;display:flex;align-items:center;justify-content:space-between;gap:16px;text-align:left;padding:24px 26px;background:white;color:var(--ink);border:0;cursor:pointer;font-weight:900;font-size:1.08rem}.faq-plus{width:38px;height:38px;border-radius:50%;display:grid;place-items:center;background:var(--cream);color:var(--s);font-weight:900;font-size:1.5rem;flex:none;transition:.25s}.faq-item.open .faq-plus{transform:rotate(45deg);background:var(--a);color:var(--p)}.faq-a{max-height:0;opacity:0;overflow:hidden;transition:max-height .32s ease,opacity .22s ease}.faq-item.open .faq-a{max-height:320px;opacity:1}.faq-a-inner{overflow:visible}.faq-a p{margin:0;padding:0 26px 26px;color:#5b6478;line-height:1.75}
      ...
      <div className="marquee" aria-hidden="true"><div className="track"><span>{data.tradeName}</span><span>{data.city}</span><span>Atención profesional</span><span>WhatsApp directo</span><span>{data.tradeName}</span><span>{data.city}</span><span>Atención profesional</span><span>WhatsApp directo</span></div></div>

      <section className="section photo-band" style={{'--band-img': `url(${data.gallery[1] || data.heroImage})`} as any}>
        <div className="wrap">
