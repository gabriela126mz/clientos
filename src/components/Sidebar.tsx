...
const NAV = [
  ...
  { href: '/dashboard/documentos', label: 'Documentos', section:'', icon: <svg fill="none"}
...
export function Sidebar({ active }: { active: string }) {
  ...
  const sessionName = profile?.nombre_contacto || profile?.nombre_negocio || user?.email?.split('@')[0] || 'Usuario'
  const businessName = profile?.nombre_negocio || 'Negocio'
